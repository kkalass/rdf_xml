...
/// Automated release script for rdf_xml
///
...
void _printUsage() {
  ...
  print('-------------');
  print('Automates the release process for the rdf_xml package.');
  print('');
  print('Usage:');
...
Future<void> _ensureNoDevVersionStrings(String releaseVersion) async {
  ...
  for (final filePath in filesToCheck) {
    ...
    final wrongPubAddPattern = RegExp(r'dart pub add rdf_xml:[^\s"]+');
    ...
    if (devPattern.hasMatch(content) || wrongPubAddPattern.hasMatch(content)) {
      ...
      content = content.replaceAll(wrongPubAddPattern, 'dart pub add rdf_xml');
